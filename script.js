...
function initFormHandling() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            // Zapisz zamówienie
            const orders = JSON.parse(localStorage.getItem('zyrasite_orders') || '[]');
            orders.unshift(orderData);
            localStorage.setItem('zyrasite_orders', JSON.stringify(orders));
    ...
    const htmlMessage = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; text-align: center; border-radius: 10px 10px 0 0;">
                <h2 style="margin: 0; font-size: 24px;">🚀 Nowe Zamówienie - ZyraSite Studio</h2>
    ...
    ofertaCards.forEach(card => {
        ...
        if (cardHeader) {
            cardHeader.addEventListener('click', function(e) {
                ...
                ofertaCards.forEach(otherCard => {
                    if (otherCard !== card) {
                        otherCard.classList.remove('expanded');
                    }
                });
                
                // Przełącz aktualną kartę
...
function loadOpinions() {
    const opinions = JSON.parse(localStorage.getItem('zyrasite_opinions') || '[]');
...
function saveOpiniaToStorage(opiniaData) {
    const opinie = JSON.parse(localStorage.getItem('zyrasite_opinie') || '[]');
    opinie.unshift(opiniaData);
    // Zachowaj maksymalnie 50 opinii
    if (opinie.length > 50) {
        opinie.splice(50);
    }
    localStorage.setItem('zyrasite_opinie', JSON.stringify(opinie));
}
...
function addOpiniaToGrid() {
    ...
    localStorage.setItem('zyrasite_opinions', JSON.stringify(opinions));
...
function checkAuthStatus() {
    const user = localStorage.getItem('zyrasite_user');
    const authButtons = document.querySelector('.auth-buttons');
    const userMenu = document.getElementById('userMenu');
    const userName = document.getElementById('userName');
...
async function handleLogin(e) {
    ...
    try {
        ...
        localStorage.setItem('zyrasite_user', JSON.stringify(userData));
...
async function handleRegister(e) {
    ...
    try {
        ...
        checkAuthStatus();
        showAuthSuccess('Konto utworzone pomyślnie! Witaj w ZyraSite Studio!');
...
async function simulateLogin(email, password) {
    ...
    const existingUsers = JSON.parse(localStorage.getItem('zyrasite_users') || '[]');
...
async function simulateRegister(name, email, password) {
    ...
    localStorage.setItem('zyrasite_users', JSON.stringify(existingUsers));
...
🤖 Powiadomienie wygenerowane automatycznie przez system ZyraSite Studio
    `;
...
function logout() {
    localStorage.removeItem('zyrasite_user');
    checkAuthStatus();
...
function loadAdminOrders() {
    ...
    const selectedOrderId = localStorage.getItem('zyrasite_selected_order');
...
function selectOrder(orderId) {
    try {
        ...
        localStorage.setItem('zyrasite_selected_order', orderId);
...
function notifyAdminAboutNewOrder(orderData) {
    ...
    const currentUser = JSON.parse(localStorage.getItem('zyrasite_user') || 'null');
...
function showClientChatWidget(orderData) {
    ...
    localStorage.setItem('zyrasite_client_order', orderData.id);
...
function loadClientChatMessages(orderData) {
    ...
    if (!order || order.messages.length === 0) {
        ...
        widgetMessages.innerHTML = order.messages.map(msg => {
            ...
            const authorName = msg.sender === 'admin' ? 'ZyraSiteStudio' : orderData.nick;
...
function notifyClient(orderId, message) {
    ...
    if (window.clientOrderId === orderId) {
        ...
        // Pokaż powiadomienie
        showNotification('Nowa wiadomość od ZyraSiteStudio!');
    }
}
...
function restoreStateAfterRefresh() {
    ...
    const clientOrderId = localStorage.getItem('zyrasite_client_order');
    if (clientOrderId) {
        const orders = JSON.parse(localStorage.getItem('zyrasite_orders') || '[]');
        ...
        if (order) {
            ...
            localStorage.removeItem('zyrasite_client_order');
        }
    }
